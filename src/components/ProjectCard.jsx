    ...
    <article className="px-3.5 sm:px-5 flex flex-col w-[18rem] xs:w-full md:flex-row  md:gap-x-12 lg:justify-center  xl:gap-x-24 ">
      ...
      >
        <div className=" h-full w-full">
          <div
            className={` ${imageOverlayClass}  absolute w-full h-full z-[20]  `}
          ></div>
      ...
      <div className="pt-8 flex justify-between xs:px-2 sm:pt-8 md:flex-col md:-order-1 md:items-center md:justify-center">
        <div className="text-xs flex flex-col gap-y-6 2xl:gap-y-6 md:mb-4 lg:text-sm  lg:mb-8">
          ...
          <ul className="uppercase font-light text-micro flex flex flex-wrap gap-2 md:text-xs 2xl:text-[1rem]">
            {terms
              .map((term) => (
                <li className="gradient py-2 px-5 rounded-full" key={term.id}>
                  {term.name}
                </li>
              ))
              .reverse()
